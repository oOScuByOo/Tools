...
import math # Ajout librairie Math
...
def AvoirLePass(): # Fonction pour avoir le mot de passe
    BOT.send("PRIVMSG "+TARGET+" :!ep1\r\n") # MP Candy pour commencer le challenge
    while 1:
        POUBELLE=BOT.recv(7000)
        print(POUBELLE)
        if POUBELLE.find("/")>-1: # Verifie le MP pour etre sur d'avoir les NB1 et NB2
            try:
                POUBELLE=POUBELLE[(POUBELLE[1:].find(":"))+2:] # Decoupe le message pour le verifier
                POUBELLE=POUBELLE[:POUBELLE.find(".")]         # Doit ressembler a nb1/nb2
                print(POUBELLE)
                nb1=int(POUBELLE[:POUBELLE.find("/")])     # Prendre le nb1
                nb2=int(POUBELLE[(POUBELLE.find("/"))+1:]) # Prendre le nb2
                REPONSE=round(math.sqrt(nb1)*nb2,2)  # Calculer la REPONSE
                REPONSE=bytes(str(REPONSE).encode("ASCII")) # Convertit en Bytes
                BOT.send("PRIVMSG "+TARGET+" :!ep1 -rep "+REPONSE+"\r\n") # Envoie la reponse a Candy
                print(BOT.recv(7000)) # Obtenir la reponse de Candy et donc le MDP
                BOT.send("QUIT :By3 By3 !") # Ferme la connection a Candy
                break
            except:
                print("[!] En attente de Challenge ...") # Si le resultat obtenue n'est pas nb1/nb2
...
USERNAME = 'V1' # Variable 2 de l'username du Bot
